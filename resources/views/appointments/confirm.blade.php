...
<div class="container mx-auto max-w-3xl px-4 sm:px-6 lg:px-8 mt-8">
    ...
    <form action="{{ route('appointments.complete', array_merge(['appointment' => $appointment->id, 'return_url' => ($returnUrl ?? request('return_url'))], $indexContext ?? [])) }}" method="POST" class="bg-white shadow-md rounded-lg p-6" id="appointment-form">
        @csrf
        @method('PUT')
        ...
        <div class="flex justify-between mt-6">
            <a href="{{ ($returnUrl ?? request('return_url')) ?: route('appointments.index', $indexContext ?? []) }}" class="px-4 py-2 bg-gray-500 text-white rounded-md hover:bg-gray-600">
                Volver
            </a>
